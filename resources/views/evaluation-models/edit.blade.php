...
@section('title', 'Editar Modelo de Avaliação')

@section('content')
    <div class="card">
        <div class="card-body">
            <div class="container-fluid p-0">
                <form action="{{ route('evaluationModels.update', $evaluationModel->id) }}" method="POST"
                    id="evaluation_model_form">
                    @method('PUT')
                    @csrf
                    <div class="d-flex align-items-center justify-content-between">
                        <h1 class="mb-4">Editar Modelo de Avaliação</h1>
                    ...
                    @include('evaluation-models.forms.form', ['evaluationModel' => $evaluationModel])
                </form>
            </div>
        </div>
    </div>
...
@section('extra-scripts')
    <script>
        $(document).ready(function() {
            let parameterIndex = {{ isset($evaluationModel) ? $evaluationModel->parameters->count() : 0 }};

            $('.parameter').each(function() {
                let parameterElement = $(this);

                parameterElement.find('.btn-add-value').on('click', function() {
                    addValue(parameterElement);
                });

                parameterElement.find('.btn-remove-parameter').on('click', function() {
                    parameterElement.remove();
                });

                parameterElement.find('.value').each(function() {
                    let valueElement = $(this);
                    valueElement.find('.btn-remove-value').on('click', function() {
                        valueElement.remove();
                    });
                });
            });
