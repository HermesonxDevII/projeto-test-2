...
@section('content')
    <div
        style="width: 100%; margin-bottom: 30px; align-items: center;"
        class="row"
    >
        <div class="col">
            <h1
                style="font-size: 36px; line-height: 48px;"
                class="p-0 m-0"
            >Pré-inscrições</h1>
        </div>

        <div class="dropdown col" style="text-align: end;">
            <button
                class="btn bg-primary btn-shadow dropdown-toggle"
                type="button"
            ...
            >
                <li>
                    <button
                        id="external-link-program-1"
                        class="dropdown-item copy-link-option"
                        style="display: flex; justify-content: center; align-items: center;"
                        data-link="{{ route('preRegistration.form', ['courses' => 'courses', 'program' => 'MentoringLearningToStudy']) }}"
                    > Mentoria Aprendendo a Estudar </button>
                </li>

                <li>
                    <button
                        id="external-link-program-2"
                        class="dropdown-item copy-link-option"
                        style="display: flex; justify-content: center; align-items: center;"
                        data-link="{{ route('preRegistration.form', ['courses' => 'courses', 'program' => 'SchoolMonitoringProgram']) }}"
                    > Acompanhamento Escolar </button>
                </li>
            </ul>
        </div>
    </div>
    ...
    <div
        class="modal fade"
        id="delete_preRegistration_modal"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer d-flex justify-content-center">
                    ...
                    <form
                        action="{{ route('preRegistration.destroy') }}"
                        method="POST"
...
@section('extra-scripts')
    <script>
         $(document).ready(function() {
            $('#external-link-program-1').on('click', function() {
        ...
        $(document).ready(function() {
            $('#external-link-program-2').on('click', function() {
                var link = $(this).data('link');
