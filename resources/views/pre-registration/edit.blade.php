...
@section('content')
    <div style="width: 100%; margin-bottom: 30px; align-items: center;" class="row">
        <div class="col">
            <h1 style="font-size: 36px; line-height: 48px;" class="p-0 m-0">Editar Pré-incrição</h1>
        </div>
        <form action="{{ route('preRegistration.update', $preRegistration->id) }}" method="POST">
            @method('PUT')
            @csrf
            <div class="card mt-5">
                <div class="card-body">
                    <div class="container-fluid p-0">
                        @include('pre-registration.forms.edit')
                    </div>
                </div>
            </div>
